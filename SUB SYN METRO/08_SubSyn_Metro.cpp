...
public:
  ...
  //    ParameterMIDI parameterMIDI;
    ...
    bool mHit = false;
    bool keyPress = false;
    string metroItalian[5] = {"Largo", "Adante", "Moderato", "Allegro", "Presto"};
    bool ctrlHit = false;
  ...
  virtual void onInit( ) override {
    ...
    bool onKeyDown(Keyboard const& k) override {
        ...
        // If shift pressed then keyboard sets preset
            std::cout << "Shift has been pressed" << std::endl;
        ...
        // * My code
            ...
            if (ctrlCheck == 1) {
            std::cout<< "Control is being held: hit 'm' for Metronome" << std::endl;
            ctrlCheck--;
            }
            else if(ctrlCheck == 2) {
            std::cout<< "Control is being held: hit 'm' to turn off Metronome, or hit 'z' or 'x' to decrease or increase the Tempo by 30 bpm" << std::endl; 
            ctrlCheck ++; 
            }
            int asciiCode = k.key();
            if (asciiCode == 109){
                if (metroOn == true) {
                    ...
                    std::cout << "Metronome Off" << std::endl;
                    ctrlCheck = 1;
                    return true;
                }
                metroOn = true;
                ...
                metroPos = 3;
                metroTempo = 120;
                std::cout<< "Metronome On: The tempo is " << metroTempo << std::endl;
                synthManager.synthSequencer().playSequence("metro120.synthSequence");
                return true;
            }
            if (metroOn == true){
            if (asciiCode == 122){
                //When z pressed
                if(metroPos == 1){
                    std::cout << "Cannot decrease Metronome any further" << std::endl;
                    return true;
                }
                metroTempo = metroTempo - 30; 
                metroPos --;
                keyPress = true;
            }
            if (asciiCode == 120){
                //When x pressed
                if(metroPos == 5){
                    std::cout << "Cannot increase Metronome any further" << std::endl;
                    return true;
                }
                metroTempo = metroTempo + 30;
                metroPos ++;
            ...
            if (keyPress == true) 
            {
              std::cout << "The tempo is " << metroTempo << ": " << metroItalian[metroPos-1] << std::endl;
              keyPress = false;
              synthManager.synthSequencer().stopSequence();
              switch(metroPos){
                case 1:
                    synthManager.synthSequencer().playSequence("metro60.synthSequence");     
                    return true; 
                case 2: 
                    synthManager.synthSequencer().playSequence("metro90.synthSequence");     
                    return true;        
                case 3:
                    synthManager.synthSequencer().playSequence("metro120.synthSequence");     
                    return true;         
                case 4:
                    synthManager.synthSequencer().playSequence("metro150.synthSequence");     
                    return true;
                case 5:
                    synthManager.synthSequencer().playSequence("metro180.synthSequence");     
                    return true;
                }
              }
            }
